...
## Исправление macOS ARM64: MyGUI

Homebrew не предоставляет формулу `mygui`, поэтому macOS job больше не пытается
установить её через `brew`. Workflow отдельно собирает MyGUI 3.4.3 из официального
репозитория MyGUI как нативную ARM64-библиотеку и добавляет каталог установки в
`CMAKE_PREFIX_PATH` основного проекта.
